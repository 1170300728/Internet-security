...
{
	u_long payload_s = 0;
	u_long LIBNET_PROTO_H = LIBNET_UDP_H;
	char payload[255];
	char err_buf[LIBNET_ERRBUF_SIZE];
	char* device = "ens33";
	u_short proto = IPPROTO_UDP;
	u_char src_mac[6] = {0x00, 0x0C, 0x29, 0xDB, 0x36, 0xAF};
	u_char dst_mac[6] = {0x00, 0x0C, 0x29, 0xDB, 0x36, 0xAF};
	char* src_ip_str = "192.168.0.104";
	...
	lib_net = libnet_init(LIBNET_LINK_ADV, device, err_buf);
	if(lib_net == NULL){
